...
MVP 외 후속 트랙 학습 목표:
- 세기 성질 발견 — V_liquid 변화에도 평형 P 동일, V_gas는 평형 도달 시간에만 영향
- 휘발성 비교 — 라울 법칙 (혼합 액체)
- 끓는점-외압 관계 (별도 시린지 셋업)
...
**시각화 추가 요소**:
- 증발·응축 카운터 동시 그래프 → 평형 시 두 곡선 수렴
- MB 분포 히스토그램 + 탈출 에너지 임계선 (high 이상)
...
후속 (시간 여유 시):
- 6.5: 세기 성질 발견 활동 UI (V_liquid 변화 비교 화면)
- 6.6: 학생 수준 검증 + README 정합화
- 6.7: 에탄올 옵션 추가
