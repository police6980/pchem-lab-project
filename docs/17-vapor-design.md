...
### 액체상·기체상 모델 (옵션 Z — 표면 추상화)

**기체상**: 보일 자유 비행 + impulse exchange. KE HSB 매핑. 중력 X.

**액체상**:
- 영역 자체는 정적 — 박스 하부에 반투명 진청 채움 (`liquid_fill_color`). 내부 입자 렌더 X.
- 표면 띠 (`surface_band_px`) 안에 표면 입자 (`N_surface_particles`) 만 가시화. 약한 KE 진동 (`surface_speed_scale`), 입자-입자 충돌 미적용 가능, 중력 X.
- 표면 입자 위 경계 탈출 = 증발 (KE > E_escape AND p_evap). 다른 경계는 튕김.
- 기체 입자가 표면선 충돌 + 응축 조건 만족 시 흡수 → 표면 입자 신규 (표면 띠 안 무작위).

**배제된 대안**:
- 액체상 입자 전체 자유 이동 + 중력 (직전 6.1-b'' 모델): 분자간 인력 부재 시 모든 입자가 바닥에 깔림. 액체 거동 위배 (사용자 검증에서 발견).
- 격자 + 진동: 고체 인상.
- LJ 인력: N² 부하 + 시각 가치 대비 구현 부담↑. 표면 추상화로 같은 학습 효과 달성.

**핵심 학습 단서**: 액체 내부는 추상화 (반투명 채움), 표면 = 사건 발생 영역으로 시선 집중. 교과서 그림 (가)(나)(다) 동역학과 시각 정합.
