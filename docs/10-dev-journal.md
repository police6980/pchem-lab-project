...
### 2026-05-21 — 검사지 구글폼 배포 + 응답 수집 개시

#### 한 일
- `google-form-script.gs`(Apps Script)로 "기체에 대한 개념 조사" 구글 설문 생성·배포. 응답자 이메일 수집 옵션 ON.
- 응답 시트 연동 = `measure/기체에 대한 개념 조사(응답).xlsx`(현재 헤더 행만, 응답 0건). 설문 문항 백업 PDF = `measure/기체에 대한 개념 조사 - Google Forms.pdf`.
- 선행 문헌 참조표 캡처 — GCT(Gas Concept Test) 오개념 분류표(Table 4.2) `measure/KakaoTalk_20260521_212811758.png`. 본 검사지 7 오개념 ↔ GCT 문항 대응 점검용.
- `.gitignore`에 `~$*`(MS Office 잠금/임시 파일) 추가.
...
### 2026-05-23 — 레포 파일 관리 정책 제도화

#### 한 일
- `CLAUDE.md`에 "파일 관리 규칙"(무날짜 파일명·버전 스냅샷 규약·`history/` 이관) + 동기화 클러스터 표 신설.
- 동기화 깨짐 정정: `검사지-수정메모.md` 상단 대상 포인터(`검사지-신규문항`→`검사지-최종문항`, 설계계획 `history/` 경로 반영) + `google-form-script.gs` 주석 속 옛 파일명.
- `.gitignore`에 브라우저 자동화 산출물(`.playwright-mcp/`, `site-landing.png`) 추가.

#### 결정: 무날짜 파일명 + 동기화 클러스터 레지스트리

**배경**: 검사지 산출물이 정본·생성물(DOCX)·배포본·구글폼 스크립트·종합문서로 다분화 → 파일명에 날짜를 넣거나 일부만 수정하면 참조 깨짐·버전 불일치 위험.

**결정**: 살아있는 마스터·생성물은 무날짜 고정 이름(날짜는 파일 안 `최종 수정` 줄 + git이 기록), 버전 동결본만 `YYYY-MM-DD_종류_vN`. 함께 움직이는 파일은 CLAUDE.md 클러스터 표로 묶어 수정 시 클러스터 전체를 동기화.

**근거**: 날짜 파일명은 수정마다 그 파일을 가리키는 모든 참조를 깨뜨림. 클러스터 표가 "원본↔생성물↔인덱스" 동기화 누락을 구조적으로 방지.

**배제된 대안**: 날짜별 파일 다발(참조 추적 불가) / 수동 기억 의존 동기화(누락 빈발).

### 2026-05-23 — 지연 동기화(deferred sync) 도입

#### 결정: 클러스터 전파를 즉시→`/sync` 일괄로 지연

**배경**: 동기화 클러스터 한 곳을 고칠 때마다 형제·생성물 전부를 즉시 재빌드하면 토큰 낭비.

**결정**: 평소엔 요청 파일만 고치고 루트 `SYNC-PENDING.md`에 재동기 필요분만 한 줄 자동 기록, 전파·재빌드·검증은 `/sync`(`.claude/commands/sync.md`) 명령 시 일괄 처리 후 그 줄 삭제. grouping(클러스터 표)은 유지하고 timing만 지연하며, 이 규칙이 "즉시·전부" 문구보다 우선. 커밋 직전 대기 목록이 비어있지 않으면 경고.

**배제된 대안**: 매 수정 즉시 전파(토큰 낭비) / 수동 기억 의존 동기화(누락 빈발).

---

## Phase 8 — 검증·테스트 정비 및 문서 정합 (2026-05-26)

### 2026-05-26 — dalton 충돌 검증 평형 미도달 해소 + 테스트 사본 동기화 등록 + 문서 재정합

#### 한 일
- `tests/dalton-collision-test.js` 디버깅: 검증 7의 `passCount++` 누락(7/7여도 6/7로 표시) 수정 + 결과 기반 `process.exit()` 부여(이전 항상 0 → 반복 실행 시 실패 미감지).
- dalton 검증 3·4(equipartition·|v|비율) flaky 진단: 30회 반복 집계로 "한쪽 편향"이 아니라 고분산(±15%대)임을 확인.
- 통계 강화로 해소: 입자 50+50→200+200, step 10000→15000, 평형 측정창 마지막 1000→5000(`EQ_WINDOW` 상수화). 60+런 전부 7/7 PASS, 검증3 비율 평균 0.997 / 검증4 오차 <1%.
- 테스트 5패스 드리프트 감사: protocol·sensor-guard·dalton 사본이 실제 소스(`protocol.js`·`serial.js`·`main.js`)와 현재 동기 상태임을 전수 확인(`median3` 바이트 일치 등).
- `CLAUDE.md` 동기화 클러스터 표에 테스트 사본 등록(펌웨어 프로토콜 클러스터 + "입자 충돌 물리" 신규).
- 문서 정합: `docs/08` § 9 갱신(7/7 반영 + 중복 § 9 번호 정렬). 프로젝트 인덱스 클러스터(`docs/06`·`docs/09`·`README.md`·`docs/00`) 재정합 — Phase 6.4 추가 / Phase 7 검사지 재정의(샤를·게이뤼삭 이연) / firmware-esp32 main 머지·단일 main 반영 / 00은 원설계 개념 Phase와 실행 Phase 분리 명시.

#### 결정: flaky 해소를 허용오차 완화가 아닌 통계 강화로

**배경**: dalton 검증 3·4가 30회 중 다수 FAIL. 단일 random seed에 ±10% 하드 판정.

**결정**: 허용오차를 늘리지 않고 입자 수·스텝·평형창을 키워 equipartition을 견고히 재현(평균 0.997).

**근거**: 이 테스트는 논문의 물리 타당성 1차 자료. 허용오차를 늘려 통과시키면 통계가 아니라 기준을 무르게 만드는 것이라 과학적으로 부정직. 입자↑는 충돌 빈도↑로 평형을 가속하는 물리적으로 정직한 수정.

**배제된 대안**: 허용오차 완화(타당성 훼손) / RNG 시드 고정(단일 시드만 검증) / 다중 시드 평균(테스트 구조 변경 비용).

#### 결정: 입자 300이 아닌 200으로 확정

**배경**: 200+200(30/30 PASS, 검증3 최악 1.083)에서 마진을 더 확보하려 300+300도 시험.

**결정**: 200+200으로 되돌림(런타임 5.6s).

**근거**: 200→300으로 늘려도 검증3 꼬리가 1.083→1.078로 거의 안 줄어듦 확인 → 잔여 분산은 1/N(공간 평균)이 아니라 유한계 열적 요동(실재 물리). 즉 입자를 더 늘려도 비용(8.5s)만 들고 마진 이득 없음.

**배제된 대안**: 300+300 유지(비용 대비 무이득) / 더 큰 입자 수(런타임 과대).

#### 결정: 테스트는 import 대신 사본 + 클러스터 등록

**배경**: 세 테스트 모두 소스를 복사(브라우저 전역 의존 회피). 소스 변경 시 자동 드리프트 미검출 위험.

**결정**: 복사 패턴 유지하되 동기화 클러스터 표에 등록해 "소스 변경 시 사본 재복사"를 상기.

**근거**: import 리팩터는 브라우저 전역 의존 제거 비용이 큼. 현 시점 사본은 전수 감사로 동기 확인됨. 클러스터 표 등록이 저비용 드리프트 추적 수단.

**배제된 대안**: import로 리팩터(비용 과대) / 등록 없이 방치(드리프트 추적 불가).

### 2026-05-26 — 원격 브랜치 정리 (아카이브 태그 후 삭제)

#### 한 일
- 원격 브랜치 12개 반영 여부 정밀 분석: `git rev-list --left-right --count`로 main 대비 ahead/behind, ahead>0인 3개(`phase3-real-sensor`·`phase5.9-stabilization`·`tutor-D-series`)는 `git cherry`로 패치 동등성 확인.
- 판정: 미반영 코드 없음 — 고유 SHA는 main 통합 일지로 대체된 `docs` 커밋이거나, cherry-pick으로 main에 반영된 기능(예: `d6c6dbe` Vernier 자동 안정화, 충돌 해소로 SHA만 상이).
- 삭제 전 12개 브랜치 tip을 `archive/<이름>` 경량 태그로 보존·푸시(슬래시는 `-`로 평탄화) → 원격 브랜치 12개 삭제. 로컬·원격 모두 `main` 단일화.

#### 결정: 직접 삭제가 아닌 "아카이브 태그 후 삭제"

**배경**: 원격 브랜치 삭제는 되돌리기 어려운 외부 작업. 분석상 코드 손실은 없으나 작업 이력(특히 superseded 문서 커밋의 맥락)은 연구 프로젝트 1차 자료로 가치.

**결정**: 각 브랜치 tip을 `archive/*` 태그로 영구 보존·푸시한 뒤 원격 브랜치만 삭제. 복구는 `git branch <이름> archive/<이름>`.

**근거**: 태그는 브랜치 목록을 어지럽히지 않으면서 ref·이력을 영구 보존. 잘못 삭제해도 무손실 복구 가능. 브랜치 리스트는 깨끗이 비움(현행 작업은 전부 main).

**배제된 대안**: 직접 삭제(복구 시 reflog/SHA 수동 추적 필요, 시간 지나면 GC 위험) / 삭제 보류(잔존 브랜치 12개가 현행 상태 오인 유발).
