...
메시지 식별자 `t`를 최상위에 두고, 타입별로 서로 다른 페이로드를 싣는다. 브라우저는 `t` 값에 따라 라우팅하며, 알 수 없는 `t`는 무시하고 경고 로그만 남긴다. 단위 규약은 v1.0과 동일 (압력은 **kPa**, 온도는 **celsius**, 부피는 **mL**).

#### 펌웨어 → 브라우저

**(1) `"t":"d"` — 데이터 프레임** (v1.0 데이터의 타입-태그 버전)

```json
{"t":"d","sensor":"pressure","value":101.3,"unit":"kPa","timestamp":1234}
```

필드 의미는 v1.0과 동일. 주기적 측정값 스트림.

**(2) `"t":"s"` — 상태/연결** (hello + heartbeat)

```json
{"t":"s","sensor":"BMP280","fw":"1.1.0"}
```

| 필드 | 타입 | 설명 |
|---|---|---|
| `sensor` | 문자열 | 센서 모델명 (예: `"BMP280"`, `"SEN0257"`, `"DS18B20"`) |
| `fw` | 문자열 | 펌웨어 버전 semver |

연결 직후 최초 1회 전송 (hello 핸드셰이크). 이후 30 초 주기 heartbeat 선택 사항. 브라우저는 이 메시지로 연결 성공 판정 및 센서 라벨 표시.
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `p0` | 숫자 | 영점 보정 후 기준 압력 (kPa) |

브라우저의 `"t":"calib"` 요청에 대한 응답. 캘리브레이션이 성공했음을 알리고 새 영점값을 회신.
...
응답으로 펌웨어는 즉시 `"t":"s"` 를 1회 송신해 생존 확인. 브라우저는 연결 복구·재접속 탐지에 사용.
...
영점 보정 개시. 펌웨어는 현재 정적 압력을 `p0`으로 저장하고 `"t":"c"` 로 ACK.
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `rate` | 정수 | 데이터 프레임 송신 주기 (ms). 50 ~ 2000 범위 권장 |

펌웨어는 `rate` 저장 후 새 주기로 `"t":"d"` 송신. ACK는 선택 사항 — 있을 경우 `{"t":"s",...}` 형태로 재전송.
