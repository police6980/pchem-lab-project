...
### 액체상·기체상 모델 분리 (Phase 6.1-b'' 결정)

**기체상**: 보일 자유 비행 + impulse exchange. KE HSB 매핑.

**액체상**: 자유 이동 + 빽빽한 무작위 분포 + 저 KE (`liquid_speed_scale` ≈ 0.2). 격자·진동 모델 미채용 — 고체 인상 회피. 균일 중력으로 자연 sedimentation.

**표면 입자 시각 강조**: 표면 근처 (`surface_band_px`) 액체 입자는 점 크기 ↑ + 약간 밝은 톤 (`liquid_surface_color`) — 탈출 후보군 시선 유도, "표면에서 사건이 발생" 학습 직관 보강.

두 모델 간 전환은 증발/응축 사건이 매개 (위상 통과 알고리즘 항목 참조).

**배제된 대안**:
- 같은 입자 모델 + 영역만 분리: 액체 입자도 통통 튀어 학생이 "액체" 로 인식 못 함. 학습 목표 ① 전달 실패.
- 격자 + 진동: 고체 인상. 액체 거동 위배.
- 표면 입자만 가시: 액체 내부 분자 운동 정보 손실.
